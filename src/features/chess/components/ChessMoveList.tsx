 ...
 *
 * @param props - Ordered chess moves.
          ...
          <div className="grid grid-cols-[3rem_1fr_1fr] gap-y-1 text-sm">
            {rows.map(({ white, black }) => (
              <div key={white.id} className="contents">
                <div className="rounded-l-lg bg-surface-2 px-2 py-2 text-muted-foreground">{white.moveNumber}.</div>
                <div className="bg-surface-2 px-2 py-2 font-medium">{white.san}</div>
                <div className="rounded-r-lg bg-surface-2 px-2 py-2 font-medium">{black?.san ?? ''}</div>
              </div>
            ))}
          </div>
        )}
