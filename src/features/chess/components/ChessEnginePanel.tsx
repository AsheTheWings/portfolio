        ...
        {analysis.lines.length === 0 && !analysis.error ? (
          ...
          <div className="space-y-2">
            {analysis.lines.map((line) => (
              <div key={line.multipv} className="rounded-xl bg-surface-2 p-3">
                ...
                <p className="mt-2 break-words font-mono text-xs text-muted-foreground">{formatPrincipalVariation(line)}</p>
              </div>
            ))}
          </div>
        )}
