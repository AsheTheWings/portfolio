...
import type { SessionEvent, SessionComponent, SessionComponentType, SessionComponentControls } from '../types';

/**
 * Event type → Component type mapping
 */
const EVENT_TO_COMPONENT_TYPE: Partial<Record<SessionEvent['type'], SessionComponentType>> = {
 ...
 */
function getControls(eventType: SessionEvent['type'], isBackground?: boolean): SessionComponentControls | undefined {
 ...
 */
function getRole(event: SessionEvent): 'user' | 'agent' | 'system' {
...
/**
 * Convert a SessionEvent to SessionComponents
 * 
 ...
 * @returns Array of SessionComponents (empty if event doesn't map)
 */
export function toSessionComponents(event: SessionEvent): SessionComponent[] {
  // Collect additional components from tool-effects sessionComponents
  let additionalComponents: SessionComponent[] = [];
  
  if (event.type === 'tool-effects') {
    const effectComponents = (event.data as { toolEffects?: { sessionComponents?: SessionComponent[]; metadata?: unknown } })?.toolEffects?.sessionComponents;
    if (Array.isArray(effectComponents)) {
      // Enrich each component with turnMetadata from event metadata
      additionalComponents = effectComponents.map((comp: SessionComponent) => ({
        ...comp,
        data: {
          ...comp.data,
  ...
  const mainComponent: SessionComponent = {
    id: componentId,
    role: getRole(event),
    type: componentType,
    isStreaming,
